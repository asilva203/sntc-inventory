...
def main():
    ...
    file = open('Output/{}-Switch Modules-{}.csv'.format(custName,datetime.now().strftime('%Y%m%d%H%M%S')),'w')
    file.write('Parent Hostname,Parent Sys Name,Parent PID,Parent Serial,Parent Product Name,Parent Product Family,Module Product Family,Module Serial,Module PID,Reachability\n'.format())
    for switch in invData:
        if invData[switch]['Modules']:
            for m in invData[switch]['Modules']:
                file.write('{},{},{},{},{},{},{},{},{},{}\n'.format(invData[switch]['Hostname'],invData[switch]['Sys Name'],invData[switch]['Product ID'],invData[switch]['Serial'],invData[switch]['Product Name'],invData[switch]['Product Family'],m['productFamily'],m['serialNumber'],m['productId'],invData[switch]['Reachability']))
        else:
